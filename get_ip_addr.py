...
def get_ip_addr2():	
	return str(sp.Popen(cmd_str, shell=True, stdout=sp.PIPE).communicate()[0])[2:-3]

def get_ip_addr3():
	return str(sp.check_output(cmd_str, shell=True))[2:-3]

"""
def get_ip_addr4():		## Most secure yet most verbose (doesn't use shell for pipes)
...
if __name__ == "__main__":
	if os.path.isfile(script_path) and file_is_executable(script_path): 	## Check that script exists & is executable
		...
		if python_version_is_at_least(3.7):
