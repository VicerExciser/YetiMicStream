...
class YetiManager():
	def __init__(self, mic_number):
		...
		self.audio_process = Process(target=self.audio_processing_loop, args=(self.hash_queue,))  #(self.post_queue,))   # args=(self.frame_q, self.p, self.duration_lock, self.do_calibration_flag, self.calibration_lock))
	   
		print(f'[{self.__class__.__name__}]  Initializing Hash Process')
	...
	def audio_processing_loop(self, hash_q):  #post_q):
		...
		# try:
			...
			clip_start_time = _get_timestamp()
			ts = time.time()
			...
			if not os.path.isfile(old_audio_clip_name):
				...
				# self.end_time = ''
				continue
			hash_q.put((old_audio_clip_name, clip_start_time, clip_end_time))
	...
	def hash_rename(self, hash_q, post_q):  #old_filename):
		...
		while True:
			while not hash_q.empty():
				...
				print('[hash_rename] Hash process received a new file')
	...
	def post_cdn(self, post_q):
		...
		while True:
			while not post_q.empty():
				message = post_q.get()
				print('[post_cdn] Posting process received a new file')
